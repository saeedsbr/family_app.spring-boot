...
public class CommitteeService {
    ...
    public CommitteeResponse createCommittee(CommitteeRequest request, UUID organizerId) {
        ...
        if (request.getMemberEmails() != null && !request.getMemberEmails().isEmpty()) {
            ...
            for (String email : request.getMemberEmails()) {
                if (cycle > committee.getTotalMembers())
                    break;
        ...
        return mapToResponse(committeeRepository.findById(committee.getId()).orElseThrow());
    }
    ...
    @Transactional
    public CommitteeTransactionResponse markPaid(UUID committeeId, CommitteePaymentRequest request, UUID organizerId) {
    ...
    public CommitteeTransactionResponse distributePot(UUID committeeId, UUID organizerId) {
        ...
        BigDecimal potAmount = committee.getAmountPerMember().multiply(BigDecimal.valueOf(committee.getTotalMembers()));
    ...
    private CommitteeResponse mapToResponse(Committee committee) {
        return CommitteeResponse.builder()
                ...
                .members(committee.getMembers().stream().map(this::mapToMemberResponse).collect(Collectors.toList()))
                .build();
    }
